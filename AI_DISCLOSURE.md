...
- This convention applies from **2026-07-10** onward; it is not claimed retroactively.

## Human review

- Changes to `main` are only possible via pull request — enforced by an active GitHub
  branch ruleset.
- CI runs on every pull request and push to `main` (`.github/workflows/build.yml`):
  ...
  (`xcodebuild test`), and a release build. CI execution is guaranteed; a passing build
  is not currently enforced as a merge requirement.
...
- **Commit trailer:** commits with AI involvement carry a `Co-Authored-By:` trailer
  ...
  `Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>` — match on the email
  address. When this convention was introduced (2026-07-10), the history consisted of
  a single initial commit without the trailer; it applies to commits made from then on.
- **File header (optional, forward-looking):** files may declare their level in a
  header comment using the repository's primary language (Swift); existing files are
...
Last updated: 2026-07-10 · Maintainer: [@marsvogel](https://github.com/marsvogel)
